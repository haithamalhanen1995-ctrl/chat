...
export const AdminPanel: React.FC<AdminPanelProps> = ({ adminUser, onLogout, onEnterGroupChat, onEnterPrivateChat }) => {
  // Database States
  ...
  return (
    <div className="w-full min-h-screen bg-zinc-950 text-white font-sans flex flex-col md:flex-row-reverse" dir="rtl" id="admin-panel-layout">
      ...
      <div className="w-full md:w-64 bg-zinc-900 border-b md:border-b-0 md:border-l border-zinc-800 p-5 flex flex-col justify-between" id="admin-sidebar">
        <div className="space-y-6">
          <div className="flex items-center gap-3 pb-5 border-b border-zinc-800">
            ...
            <div>
              <h2 className="font-bold text-sm leading-tight text-white">لوحة تحكم المالك</h2>
              <p className="text-[11px] text-zinc-500 mt-0.5">شات دجلة الفوري</p>
            </div>
          </div>
          ...
          <nav className="space-y-1.5 flex flex-col" id="admin-nav-links">
            ...
            >
              <FolderPlus size={18} />
              إدارة الكروبات ({statsGroupsCount})
            </button>
            <button
              id="admin-tab-users"
            ...
            >
              <Users size={18} />
              إدارة المستخدمين ({statsUsersCount})
            </button>
            <button
              id="admin-tab-dms"
      ...
      <div className="flex-1 p-6 md:p-8 overflow-y-auto max-h-screen" id="admin-main-stage">
        ...
        {activeTab === 'stats' && (
          <div className="space-y-8" id="tab-content-stats">
            <div>
              ...
              <p className="text-zinc-400 text-xs mt-1">مراقبة فورية لنشاط وأداء منصة شات دجلة</p>
            </div>
        ...
        {activeTab === 'owner_settings' && (
          <div className="space-y-8" id="tab-content-owner-settings">
            <div>
              <h1 className="text-2xl font-bold font-sans text-white">إعدادات حساب المدير</h1>
              <p className="text-zinc-400 text-xs mt-1">تغيير اسم المستخدم وكلمة المرور الخاصة بلوحة تحكم المالك (الأدمن)</p>
            </div>

            <div className="max-w-md bg-zinc-900 border border-zinc-800 rounded-2xl p-6" id="owner-settings-card">
