...
export const ChatView: React.FC<ChatViewProps> = ({
  ...
  // 4. Mark private messages as read when viewing DMs
  useEffect(() => {
  ...
  }, [isSelectedDmActive, privateMessages, currentUser.id]);
  ...
  }, [selectedGroup?.id, isSelectedDmActive, currentUser.id]);
    ...
    try {
      if (profileImageFile) {
        try {
          ...
          console.warn('Firebase Storage upload failed, using local Base64:', storageErr);
          finalPhotoUrl = profileImagePreview; // Fallback to base64 preview
        }
      }
  ...
  // Listen for private DMs with admin
  useEffect(() => {
    ...
    return () => unsubDms();
  }, [currentUser.id, currentUser.role, selectedContact?.id]);
  ...
  return (
    <div className="w-full h-screen bg-zinc-950 text-white font-sans flex flex-col md:flex-row-reverse" dir="rtl" id="chat-stage-layout">
      ...
      <div className="w-full md:w-80 bg-zinc-900 border-b md:border-b-0 md:border-l border-zinc-800 flex flex-col h-[40vh] md:h-screen shrink-0" id="chat-sidebar">
        {/* User Profile Header */}
        <div className="p-4 bg-zinc-950/80 border-b border-zinc-800 flex items-center justify-between" id="chat-user-header">
          <button
            onClick={() => setShowProfileModal(true)}
            className="flex items-center gap-2.5 text-right hover:opacity-80 transition-opacity cursor-pointer group flex-1"
          ...
          >
            ...
            <div className="text-right flex-1 min-w-0">
              <h4 className="font-bold text-xs text-white leading-tight truncate">
                {currentUserProfile?.fullName || currentUser.fullName}
              </h4>
          ...
          <button
            id="btn-user-logout"
            onClick={onLogout}
            className="w-8 h-8 rounded-lg bg-zinc-800 hover:bg-red-950/40 text-zinc-400 hover:text-red-400 flex items-center justify-center transition-colors cursor-pointer shrink-0 mr-2"
            title="تسجيل الخروج"
          >
            <LogOut size={14} />
          </button>
                      ...
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <div className="min-w-0 text-right">
                            <span className="text-xs font-bold block text-white truncate">{user.fullName}</span>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`flex flex-col ${isOwn ? 'items-start' : 'items-end'}`}>
                        {/* Sender Info */}
                        <span className="text-[10px] text-zinc-400 mb-1 font-sans px-1">
                          {isOwn ? 'أنت' : msg.senderName} <span className="text-zinc-600 font-mono text-[9px]">@{msg.senderUsername}</span>
                        </span>
                ...
                <div className="flex items-center gap-3">
                  {currentUser.role === 'admin' && selectedContact ? (
                    <>
                      ...
                      <div className="text-right">
                        <h3 className="font-bold text-sm text-white">{selectedContact.fullName}</h3>
                        <p className="text-[10px] text-emerald-400">مراسلة خاصة مع العضو @{selectedContact.username}</p>
                      </div>
                    </>
                  ) : (
                    <>
